...
def _unzip(path, file):
    ...
    try:
        if os.path.exists(path):
            os.chdir(path)
        zip_package = zipfile.ZipFile(file) # 被解压的文件对象
...
def _zip(src_dir, dest_dir, file_name):
    """Argue src_dir is the dir that will be compressed.
    Argue dest_dir is the dir for saving.
    Argue file_name is the zipfile name."""
    if "accountanalysis-cnpsec" in file_name:
        # 需中邮账户分析特殊处理
        base_dir = os.getcwd().split("download")[0]
        right_config_path = os.path.join(os.path.join(base_dir, "config"), "config.js")
        wrong_config_path = src_dir
        try:
            __replace_config(right_config_path, wrong_config_path)
            print("Replace config.js Successfully.")
        except Exception as e:
            print("Replace config.js Error")
            print(str(e))
...
def __replace_config(origin_config, be_replaced):
    if os.name == "nt":
