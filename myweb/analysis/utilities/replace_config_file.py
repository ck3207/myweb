...
class Replace:
    ...
    def unzip_file(self):
        ...
        try:
            ...
            print("Unzip file[%s] Successfully!" % file_name)
            ...
            if not os.path.exists(target_dir):
                os.mkdir(target_dir)
            print(src_dir, target_dir, file_name)
            return src_dir, target_dir, file_name
        except Exception as e:
            print("Unzip file[%s] failed!" % file_name)
            print("Error:", str(e))
        finally:
            zip_package.close()
    ...
    def replace_config(self, right_config_path, replaced_config_path, config_name="config.js"):
        ...
        if not (os.path.exists(right_config) and os.path.exists(replace_config)):
            print("File Not Exists [{0}], [{1}]".format(right_config, replace_config))
        if os.name == "nt":
